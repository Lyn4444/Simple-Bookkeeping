...
public class OutChartFragment extends ChartBaseFragment {

    @Override
    protected void setAxisData(int year, int month, int dayNum) {
        List<ILineDataSet> sets = new ArrayList<>();

        List<LineChartBean> dataList = DBManager.getDaySumMoneyInMonth(year, month, 0);
        ...
        if (dataList.size() == 0) {
            ...
            dataSet1.setColor(getResources().getColor(R.color.pink));  //折线的颜色
            dataSet1.setCircleColor(getResources().getColor(R.color.pink));
...
//            lineChart.setExtraOffsets(5, 5, 5, 5);
    ...
    protected void setYAxis(int year, int month) {
        float maxMoney = DBManager.getMaxMoneyByDayInMonth(year, month, 0);
...
//        不显示图例
    ...
    public void loadData(int year, int month) {
        ...
        beans = DBManager.getMothTotalMoney(year, month, 0);
        chartBeanList.clear();
